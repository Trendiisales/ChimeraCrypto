...
// ══════════════════════════════════════════════════════════════════════════════
static int harness_unused_main(int argc, char* argv[]) {
...
// ============================================================================
// ALL-191 HONEST RE-VALIDATION (S-2026-06-14) — runs every wired roster config
// through the honest (SL-first) run_backtest. Reuses load_klines_from_json +
// run_backtest above.
// ============================================================================
#include <map>
#include "core/SymbolIndex.hpp"
static std::vector<chimera::EdgeEngine::Config> g_cfgs;
// g_slots-style registration: the brace-element constructor captures the config.
struct SlotStub {
    SlotStub(int, chimera::EdgeEngine* eng, const char*, long, const char*,
             double=0,double=0,long=0,long=0,long=0) { if(eng) g_cfgs.push_back(eng->cfg()); }
};
struct GSlotsSink { void push_back(const SlotStub&) {} } g_slots;
void wire_engine(chimera::EdgeEngine& e) { g_cfgs.push_back(e.cfg()); }

static std::vector<Kline> aggregate_tf(const std::vector<Kline>& h1, int tf_secs) {
    if (tf_secs <= 3600) return h1;
    int64_t tf_ms = (int64_t)tf_secs * 1000;
    std::vector<Kline> out;
    for (size_t i = 0; i < h1.size();) {
        int64_t bucket = h1[i].open_ts_ms - (h1[i].open_ts_ms % tf_ms);
        Kline k; k.open_ts_ms = bucket; k.o=h1[i].o; k.h=h1[i].h; k.l=h1[i].l; k.c=h1[i].c;
        size_t j=i;
        while (j<h1.size() && (h1[j].open_ts_ms-(h1[j].open_ts_ms%tf_ms))==bucket) {
            if(h1[j].h>k.h)k.h=h1[j].h; if(h1[j].l<k.l)k.l=h1[j].l; k.c=h1[j].c; ++j;
        }
        out.push_back(k); i=j;
    }
    return out;
}
static void collect_roster() {
// canonical compiled set per src/main.cpp:6230-6235 (s41/s42 CULLED)
#include "../src/engines_s43_repromote.cpp"
#include "../src/engines_s43b_holdout.cpp"
#include "../src/engines_gems.cpp"
#include "../src/engines_mr.cpp"
}
int main() {
    namespace fs = std::filesystem;
    std::string dd = fs::exists("data/btc_h1_part0.json") ? "data" : "backtest/data";
    collect_roster();
    std::fprintf(stderr,"[REVAL] collected %zu wired configs\n", g_cfgs.size());
    std::map<std::string,std::vector<Kline>> cache;
    auto get_h1=[&](const std::string& sym)->std::vector<Kline>&{
        auto it=cache.find(sym); if(it!=cache.end()) return it->second;
        std::string base=(sym=="btcusdt")?"btc":sym;
        return cache[sym]=load_klines_from_json(dd+"/"+base+"_h1_part0.json");
    };
    std::printf("tag,symbol,strategy,tf_secs,trades,wins,wr,net_bp,pf,sharpe,maxdd_bp\n");
    for (auto& cfg : g_cfgs) {
        auto kl = aggregate_tf(get_h1(cfg.symbol), (int)cfg.tf_secs);
        chimera::EdgeEngine e(cfg);
        auto r = run_backtest(e, cfg, kl);
        std::printf("%s,%s,%s,%d,%d,%d,%.1f,%.0f,%.3f,%.2f,%.0f\n",
            r.tag.c_str(), r.symbol.c_str(), r.strategy.c_str(), r.tf_secs,
