...
// AUTO-RECONNECT with exponential backoff - THREAD-SAFE VERSION
// =============================================================================
#pragma once
...
static constexpr int STALE_TIMEOUT_SEC = 60;         // 60 seconds before forcing reconnect
...
// =============================================================================
// BinanceWebSocket with Auto-Reconnect (THREAD-SAFE)
// =============================================================================
...
public:
    ...
    void stop() {
        ...
        // Wait for recv_loop to exit
        wait_for_recv_loop_exit();
...
private:
    ...
    std::atomic<bool> recv_loop_active_;
    ...
    void wait_for_recv_loop_exit() {
        // Wait up to 2 seconds for recv_loop to exit
        for (int i = 0; i < 20 && recv_loop_active_.load(std::memory_order_acquire); ++i) {
    ...
    bool do_connect() {
        ...
        reconnect_delay_ms_.store(RECONNECT_BASE_MS, std::memory_order_release);
    ...
    void reconnect_monitor() {
        while (running_.load(std::memory_order_acquire)) {
            // Wait for reconnect request or timeout
            {
                std::unique_lock<std::mutex> lock(reconnect_mutex_);
                reconnect_cv_.wait_for(lock, std::chrono::seconds(5), [this]() {
                    return reconnect_requested_.load(std::memory_order_acquire) || 
            ...
            if (connected_.load(std::memory_order_acquire)) {
                ...
                uint64_t stale_ns = (uint64_t)STALE_TIMEOUT_SEC * 1000000000ULL;
                
                if ((now - last_msg) > stale_ns) {
                    std::printf("[BINANCE] Connection stale (no messages for %ds), requesting reconnect\n", 
                               STALE_TIMEOUT_SEC);
                    reconnect_requested_.store(true, std::memory_order_release);
                }
            }
            
            // Handle reconnect if requested
            if (reconnect_requested_.load(std::memory_order_acquire)) {
                reconnect_requested_.store(false, std::memory_order_release);
                
                int delay = reconnect_delay_ms_.load(std::memory_order_acquire);
                std::printf("[BINANCE] Reconnecting in %d ms...\n", delay);
                
                // Wait for recv_loop to notice disconnect and exit
                connected_.store(false, std::memory_order_release);
                wait_for_recv_loop_exit();
                ...
                if (do_connect()) {
                    ...
                    // Request another reconnect attempt
                    reconnect_requested_.store(true, std::memory_order_release);
                }
            }
        }
    ...
    void recv_loop() {
        recv_loop_active_.store(true, std::memory_order_release);
        ...
        while (running_.load(std::memory_order_acquire)) {
            ...
            int n;
            {
                std::lock_guard<std::mutex> lock(ssl_mutex_);
                if (!ssl_) continue;
                n = SSL_read(ssl_, recv_buf_, sizeof(recv_buf_));
            }
        ...
        recv_loop_active_.store(false, std::memory_order_release);
    }
