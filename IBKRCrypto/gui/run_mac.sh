...
export IBKRCRYPTO_STATE="$HOME/IBKRCrypto/backtest/data/ibkrcrypto/state.json"
...
export IBKRCRYPTO_STATE_INTRADAY="$HOME/IBKRCrypto/backtest/data/ibkrcrypto_intraday/state.json"
export COMPANION_STATE_INTRADAY="$HOME/IBKRCrypto/companion_intraday/companion_state.json"

HERE="$HOME/IBKRCrypto/gui"
LOG="/tmp/crypto_gui_mac.log"
PY=/usr/bin/python3

PAT="IBKRCrypto/gui/server.py"   # full-path match (launched below with full path)
...
echo "[$(date '+%F %T')] crypto GUI (re)started on 127.0.0.1:8090 (state=$IBKRCRYPTO_STATE)" >>"$LOG"
