...
public:
                  ...
                  double available_R)  // From risk governor
    {
        ...
        if (should_exit) {
            double final_pnl = pnl_bp * pos.size_R;  // Scale by size
            total_pnl_bp += final_pnl;
            total_trades++;
            if (final_pnl > 0) winning_trades++;
            
            std::printf("[STRUCTURAL-EXIT] %s | pnl=%.2fbp | size=%.1fR | mfe=%.1f | mae=%.1f | reason=%s | total=%.1fbp\n",
                symbol.c_str(), final_pnl, pos.size_R, pos.mfe_bp, pos.mae_bp, 
