...
int main() {
    ...
    // ── S-2026-07-20 MEASURED COST FEED (operator order: correct cost, never hand-quoted) ──
    // Feed the DepthLiquidationModel per-coin p99 entry/exit slips (data/depth_cost_measured.csv,
    // regenerated by backtest/depth_cost_model_bt over data/bookdepth_perp) into the cost ledger
    // BEFORE any companion config reads safe_cost_bps(sym). safe_cost_bps = fee20 + entry_p99 +
    // exit_p99 + spread2 + latency5 + dust1 — the single authoritative RT number
    // (feedback-crypto-cost-authoritative-depth-model). Missing file = defaults stand (28.0
    // fee-decomposition, slips 0) with a LOUD boot warning, never a silent hand literal.
    {
        std::ifstream cf("data/depth_cost_measured.csv"); std::string ln; int nfed = 0;
        while (cf && std::getline(cf, ln)) {
            if (ln.empty() || ln[0] == '#' || ln.rfind("sym,", 0) == 0) continue;
            std::stringstream ss(ln); std::string sym, e, x;
            if (!std::getline(ss, sym, ',') || !std::getline(ss, e, ',') || !std::getline(ss, x, ',')) continue;
            g_camp_cost_ledger.set_depth_slip(sym, atof(e.c_str()), atof(x.c_str()));
            std::printf("[COST-MEASURED] %s entry_p99=%.1fbp exit_p99=%.1fbp -> safe_cost_bps=%.1f\n",
                        sym.c_str(), atof(e.c_str()), atof(x.c_str()),
                        g_camp_cost_ledger.safe_cost_bps(sym));
            ++nfed;
        }
        if (nfed == 0)
            std::printf("[COST-MEASURED] *** data/depth_cost_measured.csv MISSING/EMPTY — safe_cost_bps "
                        "falls back to the 28.0bp fee-decomposition default (slips unmeasured) ***\n");
        else
            std::printf("[COST-MEASURED] %d coins fed from depth model (measured, not hand-quoted)\n", nfed);
        std::fflush(stdout);
    }
    ...
    auto make_becascade_cell = [&unretired](const char* ptag, const char* ctag, const char* sym,
        ...
        c.parent_tag = ptag; c.tag = ctag; c.symbol = sym;
        c.det_w = det_w; c.det_thr = det_thr; c.tf_secs = 3600;
        c.round_trip_bp = g_camp_cost_ledger.safe_cost_bps(sym);  // S-2026-07-20 MEASURED per-coin (depth-model feed at boot; was rt=28 PROXY)
    ...
    // boot [MIMIC-FLOOR-GATE]. Vault [[EthMimic15BeCascade]] (all-coin extension owed this session).
    {
        ...
        static const std::vector<BcCell> _bc_cells = {
            // S-2026-07-20ab CULL (operator): every BECASC cell that FAILED the honest fill_px measured-cost re-cert (S-20z, 369/380 FAIL) is DELETED. Only the 11 honest survivors remain wired (mirror acquire_allowlist unchanged). Do NOT re-add failed cells without a NEW honest cert.
    ...
    // still carries pending its own per-coin re-run).
    {
        ...
        // append_1h_tails.py — fetch_coin_1h clobber trap avoided; 0 gaps, 0 continuity
        // aborts), thr {0.5,0.75,1.0,1.25,1.5,2.0}% x W{1,2,4,8,12,24} x g{0.2,0.5,0.75}:
        // ALL 33 x 108 cells PASS the standalone gate (net>0 & PF>=1.3 & both WF halves>0
...
// #include "engines_lowturn.cpp"               // S54m: 18 low-turnover trend engines (INERT — removed S-2026-07-20)
    ...
    //   live      = g_slots (21 TRENDROSTER legs, session-tag 72; incl. 4 SOL/XRP/ATOM/DOT RSIREV BE-floor S-23) + DOGE BECASC mimic allowlist (7) = 28
