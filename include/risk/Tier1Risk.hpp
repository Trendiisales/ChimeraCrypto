...
//
// ENGINE INTEGRATION (session 6 wiring)
...
//   And every engine's open/close paths call:
...
//
// SESSION 6 (2026-05-10): EngineType enum extended from 8 to 10 entries
//   to cover the full deployed portfolio:
//       SWING, FUNDING_WINDOW, BASIS_MOMENTUM, OBI,
//       ETH_BTC_LEADLAG, COINBASE_PREMIUM_MREV, FUNDING_PERSIST_FADE,
//       VOL_COMPRESSION_BREAKOUT, RANGE_MEAN_REVERSION, MULTI_SYMBOL_ROTATION
//   Persistence file format unchanged (positions[] is keyed by engine
//   name string, so old state files load forward — new engines get
...
enum class EngineType : int {
    ...
    RANGE_MEAN_REVERSION      = 8,
    MULTI_SYMBOL_ROTATION     = 9,
    NUM_ENGINES               = 10
};
...
inline const char* engine_name(EngineType e) {
    switch (e) {
        ...
        case EngineType::RANGE_MEAN_REVERSION:     return "RANGE_MR";
        case EngineType::MULTI_SYMBOL_ROTATION:    return "MULTI_SYM_ROT";
...
public:
    ...
    struct Config {
        ...
        // correlation control; N_ENGINES effectively disables.
