...
def signal(sym,csvf,cost,strat):
    ...
    try:
        out=subprocess.run([BT,sym,csvf,"--signal",strat],capture_output=True,text=True,
                           env=dict(os.environ,COSTBPS=str(cost),REGIME_MA=str(rma)),timeout=30).stdout
...
def _eqret(sym,csvf,strat):                      # trailing daily returns from --equity (cum sum-ret -> delta)
    try:
        out=subprocess.run([BT,sym,csvf,"--equity",strat],capture_output=True,text=True,timeout=30).stdout
        d={}
        for ln in out.splitlines():
...
for L in legs:
    ...
    if PROTECT:
        if key in clipped:                                 # re-open guard: hold flat until signal flips off the clip
    ...
    if t!=pos0:                                            # ---- TRADE (flip) ----
        ...
        if t!=0: epx=px; ets=now; qty=qty_for(epx,mult,scale); p["peak_usd"]=0.0; p["peak_ts"]=_unix(now); led.write(f"{now},{key},OPEN {t},{t},{px:.4f},,{cum:.3f}\n"); ntrades+=1
